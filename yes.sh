...
SCRIPT_VERSION="2026.06.22-keybinds-final-5pass-fixed-copy"
MODE="${1:-install}"
USER_NAME="${SUDO_USER:-${USER:-nexo77}}"
HOME_DIR="$(getent passwd "$USER_NAME" | cut -d: -f6 || true)"
if [[ -z "$HOME_DIR" ]]; then
  HOME_DIR="${HOME:-/home/nexo77}"
fi

TS="$(date +%Y%m%d-%H%M%S)"
STATE_DIR="$HOME_DIR/.local/state/nexo77-hypr-keybinds-final"
LOG_DIR="$STATE_DIR/logs"
CACHE_DIR="$HOME_DIR/.cache/nexo77-hypr-keybinds-final"
BACKUP_DIR="$HOME_DIR/rice-backups/nexo77-keybinds-final-$TS"
...
HYPR_TEMPLATES_DIR="$HYPR_DIR/templates"
...
banner() {
  cat <<BANNER
╔══════════════════════════════════════════════════════════════╗
║       NEXO77 HYPRLAND KEYBINDS FINAL FIX                   ║
...
usage() {
  cat <<USAGE
Uso:
  bash ./nexo77_hyprland_keybinds_final_fix.sh install   # reparación completa
  bash ./nexo77_hyprland_keybinds_final_fix.sh runtime   # reinicia solo runtime/rice
  bash ./nexo77_hyprland_keybinds_final_fix.sh doctor    # diagnóstico
  bash ./nexo77_hyprland_keybinds_final_fix.sh rollback  # restaura último backup
USAGE
}
...
choose_terminal() {
  if have kitty; then printf 'kitty\n'; return 0; fi
  if have foot; then printf 'foot\n'; return 0; fi
  if have alacritty; then printf 'alacritty\n'; return 0; fi
  if have konsole; then printf 'konsole\n'; return 0; fi
  install_one_pkg kitty
  printf 'kitty\n'
}

choose_launcher() {
  if have rofi; then printf 'rofi -show drun\n'; return 0; fi
  if have wofi; then printf 'wofi --show drun\n'; return 0; fi
  install_one_pkg rofi-wayland
  if have rofi; then printf 'rofi -show drun\n'; return 0; fi
  printf 'true\n'
}
...
comment_dangerous_tty_autostarts() {
  ...
  for file in "${files[@]}"; do
    [[ -f "$file" ]] || continue
    cp -a "$file" "$file.bak-nexo77-keybinds-final-$TS" || true
    sed -i -E \
      -e '/^[[:space:]]*(exec[[:space:]]+)?Hyprland([[:space:]]|$)/ s/^/# DISABLED_BY_NEXO77_KEYBINDS_FINAL /' \
      -e '/^[[:space:]]*(exec[[:space:]]+)?start-hyprland([[:space:]]|$)/ s/^/# DISABLED_BY_NEXO77_KEYBINDS_FINAL /' \
      -e '/^[[:space:]]*(exec[[:space:]]+)?dbus-run-session[[:space:]]+Hyprland([[:space:]]|$)/ s/^/# DISABLED_BY_NEXO77_KEYBINDS_FINAL /' \
...
find_rice_source() {
  ...
  if have git; then
    printf "\n==> No encontré cache del rice; clonando imperative-dots\n" >&2
...
deploy_rice_or_minimal() {
  ...
  if source="$(find_rice_source)"; then
    ...
    rm -rf "$HYPR_DIR"
    mkdir -p "$HYPR_DIR"
    cp -a "$source"/. "$HYPR_DIR"/ || die "No pude copiar contenido del rice desde $source"
  else
  ...
  mkdir -p "$HYPR_CONFIG_DIR" "$HYPR_TEMPLATES_DIR" "$LOCAL_BIN"
...
write_known_good_settings() {
  step "Escribir settings.conf con colores reales"
  mkdir -p "$HYPR_CONFIG_DIR" "$HYPR_TEMPLATES_DIR"
...
EOF_SETTINGS
  ...
  cat > "$HYPR_TEMPLATES_DIR/settings.conf.template" <<'EOF_SETTINGS_TEMPLATE'
input {
    kb_layout = {{KB_LAYOUT}}
...
EOF_SETTINGS_TEMPLATE

  ok "settings.conf y template reparados"
}
...
compile_templates_if_available() {
  ...
  if [[ -f "$watcher" ]]; then
    ...
    if bash "$watcher" --compile; then
      ...
      warn "settings_watcher falló; continuaré y sobrescribiré archivos limpios"
    fi
  else
...
write_keybindings() {
  step "Crear keybinds mínimos funcionales"
  mkdir -p "$HYPR_CONFIG_DIR"
  local terminal_cmd launcher_cmd
  terminal_cmd="$(choose_terminal)"
  launcher_cmd="$(choose_launcher)"

  cat > "$HYPR_CONFIG_DIR/keybindings.conf" <<EOF_KEYS
# nexo77 stable keybinds
\$mainMod = SUPER

bind = \$mainMod, RETURN, exec, $terminal_cmd
bind = \$mainMod, Q, killactive
bind = \$mainMod, D, exec, $launcher_cmd
bind = \$mainMod, M, exit
...
EOF_KEYS
  ...
  ok "Keybinds escritos usando terminal: $terminal_cmd / launcher: $launcher_cmd"
}
...
doctor() {
  ...
  {
    echo "NEXO77 HYPRLAND KEYBINDS FINAL REPORT - $TS"
    ...
    echo
    echo "Bad references scan in .conf and ~/.local/bin only:"
    { find "$HYPR_DIR" -type f -name '*.conf' -print0 2>/dev/null; find "$LOCAL_BIN" -maxdepth 1 -type f -print0 2>/dev/null; } | xargs -0 grep -InE 'login-stable\.conf|rice-rescue\.conf|windowrulev2|\$active_border|\$inactive_border|ilyamiro-rice-healer|ilyamiro-rice-runtime-(clean|safe)|Main\.qml|TopBar\.qml|Floating\.qml' 2>/dev/null || echo "No bad references found"
    echo
    echo "Processes:"
    pgrep -a -u "$USER_NAME" -f 'Hyprland|quickshell|awww-daemon|hypridle|swayosd-server|wl-paste|playerctld' || true
    echo
    echo "settings.conf relevant:"
    grep -nE 'col\.active_border|col\.inactive_border|general|input|decoration|misc' "$HYPR_CONFIG_DIR/settings.conf" 2>/dev/null || true
    echo
    echo "keybindings.conf content:"
    sed -n '1,220p' "$HYPR_CONFIG_DIR/keybindings.conf" 2>/dev/null || true
    echo
    echo "autostart.conf content:"
...
rollback() {
  ...
  local latest
  latest="$(ls -dt "$HOME_DIR"/rice-backups/nexo77-keybinds-final-* 2>/dev/null | head -n 1 || true)"
  [[ -n "$latest" ]] || die "No encontré backups nexo77-keybinds-final-*"
