...
SCRIPT_VERSION="2026.06.22-keybinds-final-4pass"
MODE="${1:-install}"
...
doctor() {
  step "Doctor/diagnóstico"
  {
    ...
    grep -RInE 'login-stable\.conf|rice-rescue\.conf|windowrulev2|\$active_border|\$inactive_border|ilyamiro-rice-healer|ilyamiro-rice-runtime-(clean|safe)|Main\.qml|TopBar\.qml|Floating\.qml' "$HYPR_DIR" "$LOCAL_BIN" 2>/dev/null || echo "No bad references found"
    echo
    echo "Processes:"
